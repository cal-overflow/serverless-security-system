<template>
  <div class="max-w-screen-lg mx-auto">
    ...
    >
      <p class="text-4xl font-bold text-center md:text-left">Settings</p>
    </div>
    <form @submit.prevent="saveUpdates">
      <div
        class="bg-card-light dark:bg-card-dark m-0 md:m-6 pb-8 md:pb-4 p-4 shadow-lg dark:shadow-shadow-dark hover:shadow-none hover:rounded motion-safe:animate-fade-in-fast transition"
      >
        <div class="w-full md:w-2/3 mx-auto">
          ...
          <div class="md:flex align-center items-center">
            <div class="w-full md:w-1/2">
              <label for="clipLength" class="font-bold text-right"
                >Clip length</label
              >
            </div>
            <select
          ...
          <div class="md:flex align-center items-center">
            <div class="w-full md:w-1/2">
              <label for="clipsPerUpload" class="font-bold text-right"
                >Clips per upload</label
              >
            </div>
            <select
              v-if="updatedConfig"
          ...
          <div class="md:flex align-center items-center">
            <div class="w-full md:w-1/2">
              <label for="defaultMotionThreshold" class="font-bold text-right"
                >Default Motion Sensitivity</label
              >
            </div>
            <select
              v-if="updatedConfig"
          ...
          <div class="flex align-center items-center">
            <div class="w-1/2 md:w-1/2">
              <label for="isMotionOutlined" class="font-bold text-right"
                >Outline Motion</label
              >
            </div>
            <toggle
              v-model="updatedConfig.isMotionOutlined"
              :show-label="true"
            />
          </div>
        </div>
      ...
      >
        <div class="w-full md:w-2/3 mx-auto">
          ...
          <div class="md:flex align-center items-center">
            <div class="w-full md:w-1/2">
              <label for="inviteUrlExpirationTime" class="font-bold text-right"
                >User Invitation Expiration Time</label
              >
            </div>
            <select
              v-if="updatedConfig"
          ...
          <div class="md:flex align-center items-center">
            <div class="w-full md:w-1/2">
              <label for="presignUrlExpirationTime" class="font-bold text-right"
                >Video URL expiration time</label
              >
            </div>
            <select
              v-if="updatedConfig"
          ...
          <div class="md:flex align-center items-center">
            <div class="w-full md:w-1/2">
              <label
                for="daysToKeepMotionlessVideos"
                class="font-bold text-right"
                >Keep motionless videos</label
              >
            </div>
            <select
              v-if="updatedConfig"
      ...
      >
        <div class="flex flex-wrap sm:flex-nowrap justify-between w-full">
          <div v-if="!hasPermissionToEdit">
            <p
              class="text-lg font-bold text-primary-light dark:text-primary-dark"
            >
              Admin permission required
            </p>
            <p class="text-md">
              Only Admin users have permission to update the system
              configuration.
            </p>
          </div>
          <p
            v-else
            class="my-auto text-extra-gray-dark dark:text-extra-gray-light"
          >
            {{
              infoMessage
                ? infoMessage
  ...
  computed: {
    hasChanges() {
      return (
        ...
        JSON.stringify(this.updatedConfig)
      );
    },
  },
  mounted() {
  ...
  methods: {
    saveUpdates() {
      ...
      updateConfig({
        ...
        presign_url_expiration_time: parseInt(
          this.updatedConfig.presignUrlExpirationTime
        ),
        invitation_url_expiration_time: parseInt(
          this.updatedConfig.inviteUrlExpirationTime
        ),
        default_motion_threshold: parseInt(
          this.updatedConfig.defaultMotionThreshold
        ),
        days_to_keep_motionless_videos: parseInt(
          this.updatedConfig.daysToKeepMotionlessVideos
        ),
