<template>
  <div class="max-w-screen-lg mx-auto">
    ...
    >
      <div
        class="md:flex align-center justify-between text-center md:text-left"
      >
        <p class="text-4xl font-bold">Footage</p>
      </div>
        ...
        <div class="min-w-max flex align-center justify-start gap-4">
          ...
          <label htmlFor="dateInput" class="relative flex items-center">
            <svg
              ...
              stroke-width="1.5"
              stroke="currentColor"
              class="z-10 pointer-events-none w-8 h-8 absolute top-1/2 transform -translate-y-1/2 left-3"
            >
              <path
                stroke-linecap="round"
                stroke-linejoin="round"
    ...
    <div
      ...
      class="bg-card-light dark:bg-card-dark m-0 md:m-6 p-4 md:p-0 md:bg-main-light md:dark:bg-main-dark grid md:grid-cols-6 justify-center sm:justify-between gap-4 md:gap-10 motion-safe:animate-fade-in-fast transition"
    >
      <video-feed-card
        :video="selectedVideo"
    ...
    <div
      v-else
      class="bg-card-light dark:bg-card-dark m-0 md:m-6 p-4 md:p-0 md:bg-main-light md:dark:bg-main-dark grid grid-cols-1 justify-center sm:justify-between gap-4 md:gap-10 motion-safe:animate-fade-in-fast transition"
    >
      <grid-card>
        <svg
          xmlns="http://www.w3.org/2000/svg"
...
export default {
  ...
  async asyncData({ params, route, error }) {
    const lastForwardSlashIndex = params.pathMatch.includes('/')
      ? params.pathMatch.lastIndexOf('/')
      : params.pathMatch.length;
    ...
    if (!validFootageTypes.includes(footageType)) {
      return error({
        ...
        error: new Error(
          `Footage type "${footageType}" is not one of ${validFootageTypes}.`
        ),
      });
    }
  ...
  computed: {
    hasChanges() {
      return (
        this.originalFilter.dateFilter !== this.dateFilter ||
        this.originalFilter.cameraFilter !== this.cameraFilter ||
        this.originalFilter.footageType !== this.footageType
      );
    },
  },
  watch: {
  ...
  methods: {
    ...
    applyChanges() {
      ...
      const isNonCameraFilterUnchanged =
        JSON.stringify(originalFilterWithoutCamera) ===
        JSON.stringify(currentFilterWithoutCamera);

      if (
        this.cameraFilter !== this.originalFilter.cameraFilter &&
        isNonCameraFilterUnchanged
      ) {
        ...
        if (!this.cameraFilter) {
          ...
          this.filteredVideos = this.videos.filter(
            ({ camera }) => camera === this.cameraFilter
          );
        this.originalFilter.cameraFilter = this.cameraFilter;
        return;
      }

      const queryParams = this.cameraFilter
        ? `?camera=${this.cameraFilter}`
        : '';
      this.$router.push(
        `/footage/${this.footageType}/${this.dateFilter}${queryParams}`
      );
    },
  },
};
