...
const defaultDateFilter = `${today.getFullYear()}-${formatNumber(
  today.getMonth() + 1
...
export const getFootage = async ({ params, route, footageType }) => {
  ...
  let dateFilter = params.pathMatch.substring(
  ...
  const [videosP1, videosP2, videosP3, cameras] = await Promise.all(
    tasks
  ).catch((err) => {
  ...
  if (startingVideoTime) {
    selectedVideoIndex = filteredVideos.findIndex(
      ({ time }) => time === startingVideoTime
    );
  }

  const selectedVideo = filteredVideos[selectedVideoIndex];
  const selectedVideoCamera = cameras.find(
    ({ id }) => id === selectedVideo?.camera
  );
