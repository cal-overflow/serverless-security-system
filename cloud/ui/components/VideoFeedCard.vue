<template>
  <grid-card
    class="w-full mx-auto px-0 py-0 sm:px-0 sm:py-0 md:p-4 border border-solid border-primary-light dark:border-primary-dark md:border-none"
  >
    <video
      id="videoPlayer1"
      controls
...
export default {
  ...
  watch: {
    video(newVideo, oldVideo) {
      const newVideoPlayer = document.getElementById(
        `videoPlayer${this.nextVideoPlayerIDNumber}`
      );
      ...
      newVideoPlayer.oncanplay = () => {
        ...
        const otherVideoPlayerIDNumber =
          this.nextVideoPlayerIDNumber === 1 ? 2 : 1;
        const otherVideoPlayer = document.getElementById(
          `videoPlayer${otherVideoPlayerIDNumber}`
        );
  ...
  mounted() {
    ...
    const videoPlaybackElementOnEndedEventHandler = () =>
      this.$emit('video-end');
