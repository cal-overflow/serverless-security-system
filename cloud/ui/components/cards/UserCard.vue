  ...
  <grid-card v-if="user">
    <svg
      ...
      :class="`w-2/3 mx-auto h-auto fill-primary-light dark:fill-primary-dark ${
        isAuthenticated ? 'text-primary-light dark:text-primary-dark ' : ''
      }`"
    >
      <path
    ...
    <div class="w-full">
      ...
      <p class="text-md">
        <span class="font-bold">Admin:</span> {{ user.admin }}
      </p>
    </div>
  </grid-card>
  <div v-else @click.prevent="invite">
    <grid-card
      class="bg-primary-light dark:bg-primary-dark text-white cursor-pointer"
    >
      <svg
        xmlns="http://www.w3.org/2000/svg"
        fill="none"
