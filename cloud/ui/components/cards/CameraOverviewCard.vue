<template>
  <div
    class="flex flex-wrap md:flex-none bg-card-light dark:bg-card-dark p-8 sm:p-4 mx-auto sm:w-full md:shadow-lg md:dark:shadow-shadow-dark hover:shadow-none hover:rounded static motion-safe:animate-fade-in-fast transition"
    ...
    <div class="text-center md:text-left mb-8 w-full">
      <p class="text-xl font-bold">
        <svg
          ...
          viewBox="0 0 24 24"
          fill="currentColor"
          class="inline w-16 md:w-8 h-auto"
        >
    ...
    <div class="w-full text-center">
      <p class="text-4xl">
        <svg
          ...
          stroke-width="1.5"
          stroke="currentColor"
          class="inline w-2/5 md:w-1/3 text-primary-light dark:text-primary-dark motion-safe:animate-fade-in"
        >
          <path
            v-if="healthyCameraCount === activeCameraCount"
      ...
      <div class="text-center my-2">
        <nuxt-link
          to="/cameras"
          class="text-primary-light dark:text-primary-dark underline hover:no-underline"
        >
          Dashboard
        </nuxt-link>
      </div>
  ...
  methods: {
    isHealthy(camera) {
      ...
      const lastUpload = new Date(
        parseFloat(camera.last_upload_time) * 1000
      ).getTime();
