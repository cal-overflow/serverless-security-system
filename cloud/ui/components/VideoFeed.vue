<template>
  <div
    class="max-w-screen-lg mx-auto bg-card-light dark:bg-card-dark m-6 p-4 shadow-lg dark:shadow-shadow-dark hover:shadow-none hover:rounded motion-safe:animate-fade-in-fast transition"
  >
    <div>
    ...
    <div class="flex flex-wrap md:flex-nowrap">
      <div class="w-full md:w-4/5 mx-auto items-start">
        ...
        <div class="w-full flex items-center justify-between">
          <div>
            ...
            >
              <svg
                ...
                stroke-width="1.5"
                stroke="currentColor"
                :class="`w-8 h-8 transition ${isFilterVisible ? 'hidden' : ''}`"
              >
                <path
                  stroke-linecap="round"
                  stroke-linejoin="round"
          ...
          <div v-if="isFilterVisible">
            <button
              :class="`rounded-full p-2 mt-2 mb-4 transition ${
          ...
          <div>
            <button
              ...
              title="Apply"
              :class="
                isChanges && isDateValid ? 'cursor-pointer' : 'cursor-default'
              "
              @click="applyFilter"
            >
              <svg
                ...
                stroke-width="1.5"
                stroke="currentColor"
                :class="`w-8 h-8 transition duration-500 ${
                  !(isChanges && isDateValid)
                    ? 'text-extra-gray-dark dark:text-extra-gray-light'
            ...
            <a
              v-if="!isFilterVisible && currentVideo"
            ...
            <button @click="next">
              <svg
                ...
                stroke="currentColor"
                :class="`w-8 h-8 ${
                  currentVideoIndex + 1 === videosFilteredByCamera.length
                    ? 'text-extra-gray-dark dark:text-extra-gray-light'
                    : ''
                }`"
              >
        ...
        <div>
          <select
            ...
            :class="`appearance-none w-full resize-none px-4 py-2 bg-extra-gray-light dark:bg-extra-gray-dark rounded-lg outline-none focus:rounded-sm focus:ring focus:ring-primary-light dark:focus:ring-primary-dark transition ${
              isFilterVisible && currentFilterOption === 'footage'
                ? ''
                : 'hidden'
            }`"
          >
          ...
          <select
            ...
            name="camera"
            :class="`appearance-none w-full resize-none px-4 py-2 bg-extra-gray-light dark:bg-extra-gray-dark rounded-lg outline-none focus:rounded-sm focus:ring focus:ring-primary-light dark:focus:ring-primary-dark transition ${
              isFilterVisible && currentFilterOption === 'camera'
                ? ''
                : 'hidden'
            }`"
          >
          ...
          <input
            ...
            name="date"
            :class="`appearance-none w-full resize-none px-4 py-2 bg-extra-gray-light dark:bg-extra-gray-dark rounded-lg outline-none focus:rounded-sm focus:ring focus:ring-primary-light dark:focus:ring-primary-dark transition ${
              isFilterVisible && currentFilterOption === 'date' ? '' : 'hidden'
            }`"
          />
          <p
            :class="`text-sm text-extra-gray-dark dark:text-extra-gray-light transition duration-250 ${
              isDateValid ? 'opacity-0' : 'opacity-1'
            }`"
          >
            Invalid date
          </p>
      ...
      <div class="w-full md:w-1/5 mx-auto pl-4">
        <div class="w-1/2 md:w-full text-lg">
          ...
          <div
            v-if="videosFilteredByCamera.length > 0"
    ...
    >
      ...
      <small
        ><strong>Note:</strong> Only users with accounts can access this
        URL.</small
      >
    </alert>
  </div>
...
export default {
  ...
  computed: {
    ...
    isChanges() {
      return !(
        this.previousFilter.type === this.type &&
        this.previousFilter.camera === this.camera &&
        this.previousFilter.dateFilter === this.dateFilter
      );
    },
    currentVideoMetadata() {
      ...
      if (this.cameras.length) {
        cameraName = this.cameras.find(
          ({ id }) => currentVideo.camera === id
        ).name;
      }
  ...
  methods: {
    getVideos(startingVideoTime) {
      ...
      for (const time of this.times) {
        fetchVideosTasks.push(
          getVideos(this.type, { ...options, hours: time })
        );
      }

      Promise.all(fetchVideosTasks)
        .then((videos) => {
          if (this.isUnmounted) {
          ...
          if (this.camera) {
            this.videosFilteredByCamera = this.videos.filter(
          ...
          if (startingVideoTime) {
            this.updateCurrentVideo(
              this.videosFilteredByCamera.findIndex(
                ({ time }) => time === startingVideoTime
              )
            );
        ...
        .then((clients) => {
          clients.forEach((client) => {
            const dateObj = new Date(
              parseFloat(client.last_upload_time) * 1000
            );
    ...
    updateCurrentVideo(index) {
      ...
      const loadingVideoElement = document.getElementById(
        `video${this.backgroundPlayerId}`
      );
      ...
      loadingVideoElement.oncanplay = () => {
        ...
        const otherVideoElement = document.getElementById(
          `video${previousVideoPlayerId}`
        );
